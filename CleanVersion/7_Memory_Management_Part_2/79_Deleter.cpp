...
{
	// new operator 대신, malloc 으로 할당한 메모리를 스마트 포인터로 관리할 수 있다
	std::unique_ptr<int> p{ (int*)malloc(4) };
	*p = 100;
	std::cout << *p << '\n';
	// smart pointer -> scope 가 끝나면서 자동으로 메모리 정리
	// malloc 으로 할당했으니까, free(p); 가 필요
...
/*
	...
	deleter 를 마음대로 설정할 수 있다 by any kind of callable
		function object
			...
			= create by overloading the function call operator on struct
		global function
		lambda expression
*/
...
void UniquePtrDeleters()
{
	...
	/* 1 */
	// Function Object 사용을 추천
	// smart pointer 에 deleter type, deleter object 을 명시 => specify deleter
	// 여기서는 Free, Free{}
	...
	*p1 = 10;
	...
	// function pointer 가 들어가면서, smart pointer 의 크기가 증가한다 => 함수 주소를 저장해야 하니까
